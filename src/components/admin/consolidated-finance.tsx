...
export function ConsolidatedFinance() {
  ...
  /** Agency (marketing) clients + the cash movements generated by their projects. */
  const { data: agency } = useQuery({
    queryKey: ["agency-clients-cf"],
    queryFn: async () => {
      const [clients, projects, incomes, expenses] = await Promise.all([
        supabase.from("agency_clients").select("id, name").order("name"),
        supabase.from("agency_projects").select("id, name, client_id"),
        supabase.from("project_incomes").select("cash_movement_id, project_id"),
        supabase.from("project_expenses").select("cash_movement_id, project_id"),
      ]);
      const projectToClient = new Map(
        (projects.data ?? []).map((p) => [p.id, p.client_id as string | null]),
      );
      const clientName = new Map((clients.data ?? []).map((c) => [c.id, c.name]));
      const byMovement = new Map<string, { clientId: string; name: string }>();
      [...(incomes.data ?? []), ...(expenses.data ?? [])].forEach((r) => {
        if (!r.cash_movement_id) return;
        const cid = projectToClient.get(r.project_id as string);
        if (!cid) return;
        byMovement.set(r.cash_movement_id as string, { clientId: cid, name: clientName.get(cid) ?? "عميل وكالة" });
      });
      return { clients: clients.data ?? [], byMovement };
    },
  });
  ...
  const matchesClient = (m: (typeof withCompany)[number]) => {
    if (contactFilter === "all") return true;
    if (contactFilter.startsWith("ag:")) return m.agency_client_id === contactFilter.slice(3);
    return m.contact_id === contactFilter;
  };
